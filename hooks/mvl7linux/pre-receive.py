...
def pre_receive(tags, branches):
	...
	if not limb.name.startswith('mvl7-'):
	    continue
...
def send_email():
    ...
    if 'internal' in bugz_dict:
	internal_branches = list(bugz_dict['internal'])
	del bugz_dict['internal']
    else:
	internal_branches = []
	...
	branches = list(bugz_dict[bugz]) + external_branches + internal_branches
...
def post_commit(commit, change, branch):
	...
	try:
		...
		pass
		notice('Warning: no from commit id in cherry-pick\n')
	...
	if change == 'deleted':
	    return
	raise Exception('No bugz for non-external or non-intrnal commit')
